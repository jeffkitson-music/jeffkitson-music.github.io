...
* Website: https://github.com/STUFFGOESHERE
*/
...
// ===== MUSIC =====
async function loadScales(filePath) {
    // Placeholder
    setPlaceholderScale();
    // Get Scales
    fetch(filePath)
...
function randomScale() {
    ...
    var scales = this.getScalesToInclude(includeScales);
    var n = getRandomNumberForScale(scales.length);
    var randomScale = scales[n];
...
function getScalesToInclude(includeScales) {
    ...
    if (includeScales.major) {
        includedScales.push(...scales.major);
    }
    if (includeScales.naturalMinor) {
        includedScales.push(...scales.natural_minor);
    }
    if (includeScales.harmonicMinor) {
        includedScales.push(...scales.harmonic_minor);
    }
    if (includeScales.melodicMinor) {
        includedScales.push(...scales.melodic_minor);
    }
...
function randomLongTone() {
    ...
    const randomIndex = Math.floor(Math.random() * longTones.length);
    var lt = longTones[randomIndex];
    if (lt.includes("^") || lt.includes("_")) {
        var lts = lt.substring(1); // lt short, change to note? ltn?
    } else {
        var lts = lt;
    }
    //var lt_abc =`X: 1\nT: Random Long Tone\n${lt}8-|${lts}8-|${lts}8-|!fermata!${lts}8|]`
...
function saveGame() {
    ...
    // Example Output: https://www.example.com/game.html?trialPointsLeft=75&trialPointsMax=100&na_left=5&na_max=10&ph_left=80&ph_max=100&d=6
    if (navigator.clipboard && navigator.clipboard.writeText) {
        ...
        Swal.fire({
            ...
            text: "Save it in notes app, text yourself, etc.!"
        });
    } else {
        Swal.fire(fullUrl);
    }
...
// ===== LOAD FUNCTIONS  =====
loadScales("https://jeffkitson-music.github.io/cnd_test/scales8.json");
initialSetup();
